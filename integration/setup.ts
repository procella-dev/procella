...
export async function truncateTables(): Promise<void> {
	...
	await sql.unsafe(
		"TRUNCATE subscription_ticket_nonces, esc_sessions, esc_environment_revisions, esc_environments, esc_projects, webhook_outbox, webhook_deliveries, webhooks, github_setup_states, github_outbound_connections, github_installations, oidc_trust_policies, update_events, journal_entries, checkpoints, updates, stacks, projects, blob_cleanup_queue CASCADE",
	);
	await sql.close();
}
