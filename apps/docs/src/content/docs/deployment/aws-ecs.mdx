...
description: Deploy Strata to AWS using ECS, PostgreSQL on EFS, and CloudFront for a cost-effective self-hosted Pulumi backend (~$150/month)
---

Strata can be self-hosted on AWS using ECS (Elastic Container Service) for the API server and PostgreSQL database, with static assets served via S3 and CloudFront. This approach is production-ready and significantly cheaper than managed alternatives.
...
┌─────────────────────────────────────────────────────────┐
│ CloudFront CDN                                          │
│ ├─ strata.strata.com (React UI, 1-year cache)         │
│ └─ docs.strata.com (Astro docs, 1-hour cache)         │
└──────────────────────┬──────────────────────────────────┘
                       │ HTTPS
┌──────────────────────┴──────────────────────────────────┐
│ ALB (Application Load Balancer)                         │
│ ├─ Health check: /healthz every 5s                     │
│ └─ Targets: ECS Strata server (port 9090)              │
└──────────────────────┬──────────────────────────────────┘
                       │ Port 9090
┌──────────────────────┴──────────────────────────────────┐
│ ECS Cluster (strata)                                    │
│ ├─ Strata Service (2-10 replicas)                      │
...
- **Strata on ECS**: ~$150/month (self-hosted)
...
- Subdomains will be auto-configured:
  - `strata.strata.com` — Web UI
  - `api.strata.com` — API server
  - `docs.strata.com` — Documentation
...
Instead of using RDS (AWS's managed relational database), Strata can use PostgreSQL on ECS with EFS storage:
...
The Strata API server automatically scales based on demand:
...
aws ecs describe-task-definition --task-definition strata-server:N

# Update service to use it
aws ecs update-service \
  --cluster strata \
  --service strata-server \
  --task-definition strata-server:N-1
```
...
2. **[Configuration Reference](../getting-started/configuration.md)** — All STRATA_* variables
3. **[Architecture Overview](../architecture/overview.md)** — How Strata works internally
4. **[Pulumi CLI](https://www.pulumi.com/docs/reference/cli/)** — Using Strata with `pulumi up`
