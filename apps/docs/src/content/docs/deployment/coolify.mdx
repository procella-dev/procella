...
services:
  procella:
    ...
    environment: &procella-environment
      ...
      # 32+ chars; the server refuses to start without it.
      PROCELLA_TICKET_SIGNING_KEY: "${PROCELLA_TICKET_SIGNING_KEY}"
  ...
  # One-shot migration gate: the server image ships /migrations, so the same
  # image applies the schema before any replica starts serving.
  migrate:
    build:
      context: .
      dockerfile: Dockerfile
  ...
  db:
    ...
    volumes:
      # postgres:18 keeps PGDATA at /var/lib/postgresql/18/docker and declares
      # the volume one level up, so the mount must be /var/lib/postgresql.
      - procella_pgdata:/var/lib/postgresql
    restart: unless-stopped
    healthcheck:
...
<Aside type="caution" title="Upgrading an existing PostgreSQL 18 service">
Earlier versions mounted `procella_pgdata` at `/var/lib/postgresql/data`, while the PostgreSQL 18 image stores its cluster under `/var/lib/postgresql/18/docker`. Before the first redeploy with this file, use the terminal on the currently running `db` service to create a `pg_dumpall` backup. Restore that backup into the database after Coolify recreates the service with the corrected `/var/lib/postgresql` mount. Keep the old container and its anonymous volume until the restored data is verified.
</Aside>
...
| `PROCELLA_TICKET_SIGNING_KEY` | `openssl rand -hex 32` output (32+ chars) |
