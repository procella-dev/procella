...
| `PROCELLA_GITHUB_APP_CLIENT_ID` | *(optional as a group)* | GitHub App client ID used for user authorization |
| `PROCELLA_GITHUB_APP_CLIENT_SECRET` | *(optional as a group)* | GitHub App client secret used for user token exchange and revocation |
...
For the optional GitHub App integration, use a dedicated App and configure all five `PROCELLA_GITHUB_APP_*` credential variables; omit all five to disable it. Configure its user authorization callback as `/github/oauth/callback` and grant read-only organization Members permission. During setup, a Procella administrator enters the exact GitHub account login and installs the App first. Procella then uses a browser-bound OAuth callback to verify through GitHub that the authorizing user can access that installation and either owns the personal account or actively administers the organization before saving the binding. SST deployments additionally require `PROCELLA_GITHUB_APP_ENABLED=true`; unset or `false` keeps the integration disabled and removes retained SST secret links. GitHub Actions reads this non-secret opt-in from the deployment environment's variables. Procella derives the App's public slug from GitHub before installation, so App renames require no deployment configuration change. Do not reuse the Renovate App credentials; it requires broader repository permissions and should remain scoped to dependency updates. After moving this repository to another GitHub organization, connect the Procella App to the destination tenant and grant it access to this repository. See the [GitHub App setup guide](apps/docs/src/content/docs/features/github-app.md).
